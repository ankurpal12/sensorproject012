...
from flask import request # this is used to get the file from the user. flask is a web framework.
...
class PredictionPipelineconfig:
    ...
    # we don't need to create a separate folder for prediction artifacts like we did for training because prediction is a one-time thing.
...
class PredictionPipeline:
    ...
    def get_predicted_dataframe(self, input_dataframe_path: pd.DataFrame):
            ...
            try:
                ...
                # removing unnamed: 0 column if it is present in the input csv file.
                # we can add condition as well to check if the column is present or not.

                predictions = self.predict(input_dataframe)

                input_dataframe[prediction_column_name] = [pred for pred in predictions]
                # adding a new column to the input dataframe with the predicted values.

                target_column_mapping = {0: 'bad', 1: 'good'}

                input_dataframe[prediction_column_name] = input_dataframe[prediction_column_name].map(target_column_mapping)
                # mapping the predicted values to 'bad' and 'good'

                os.makedirs(self.prediction_pipeline_config.prediction_output_dirname, exist_ok=True)
                # creating the prediction output directory if it doesn't exist.

                input_dataframe.to_csv(self.prediction_pipeline_config.prediction_file_path, index=False)
                # saving the dataframe and converting it to csv file.
